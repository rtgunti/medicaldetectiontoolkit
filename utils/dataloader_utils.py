...
def get_test_generator(cf, logger):
    ...
    if cf.hold_out_test_set:
        ...
        train_ix, val_ix, test_ix, _ = fold_list[cf.fold]
        print(train_ix, val_ix, test_ix)
        logger.warning('WARNING: using validation set for testing!!!')
        test_ix = val_ix if not test_ix else test_ix  ## Use val if test is empty
...
#             out_targets = np.array(np.repeat(batch_class_targets, out_data.shape[0], axis=0))
            out_targets = [0] * out_data.shape[0]  # I have no idea what's happening here
