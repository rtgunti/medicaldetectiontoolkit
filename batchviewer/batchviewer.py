...
class BatchViewer(QtGui.QWidget):
    def __init__(self, parent=None, width=300, height=300):
        ...
        self.batch = None
        self.b = QtGui.QPushButton('button')
        self.width = width
        self.height = height
        self.slicingWidgets = {}
        self._init_gui()
    ...
    def btnstate(self):
        for i in range(self.batch.shape[0]):
            self.batch[i] = self.batch[0]
        self.setBatch(self.batch)
    ...
    def setBatch(self, batch, lut={}):
        ...
        self._my_layout.addWidget(self.b, 0, 2)

    def _init_gui(self):
        self.b.clicked.connect(self.btnstate)
...
def view_batch(*args, width=300, height=300, lut={}):
    ...
    print("Use these ", use_these.shape)

    sv.show()
    # sv.b.clicked.connect(btnstate(sv, use_these))
    app.exit(app.exec_())


# def btnstate(sv, use_these):
#     for i in range(sv.batch.shape[0]):
#         sv.batch[i] = sv.batch[0]
#     print("Hell", sv.batch.shape)
#     sv.setBatch(sv.batch)
