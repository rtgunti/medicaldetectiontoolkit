...
#                                           false_positive_weight=float(self.cf.fp_dice_weight))
        ...
        if self.cf.seg_loss_mode == 'wce' or self.cf.seg_loss_mode == 'dice_wce':
            ...
            loss += ce_loss
            loss_str += "ce_loss: " + str(round(ce_loss.item(),3)) 
            results_dict['monitor_values']['ce_loss'] = ce_loss.item()
...
#         results_dict['logger_string'] = "loss: {0:.3f} ce loss : {1:.3f} dice_loss : {2:.3f} dice_score {3:.3f}"\
#         .format(loss.item(), ce_loss.item(), 1 - batch_dice_score.item(), batch_dice_score.item())
        results_dict['logger_string'] = "loss: {0:.3f} {1}"\
        .format(loss.item(), loss_str)
